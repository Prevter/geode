...
static enumKeyCodes keyToKeyCode(int code) {
    switch (code) {
        ...
        case UIKeyboardHIDUsageKeyboardEqualSign: return enumKeyCodes::KEY_OEMEqual;
...
// iOS dooesn't have repeat built in, so we simulate it
- (void)startRepeat:(UIKey*)key {
    ...
    delayTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 repeats:NO block:^(NSTimer * _Nonnull t) {
        ...
        repeatTimer = [NSTimer scheduledTimerWithTimeInterval:0.05 repeats:YES block:^(NSTimer * _Nonnull t2) {
            ...
            if (KeyboardInputEvent(code, Repeat, {static_cast<uint64_t>(key.keyCode), 0}, std::chrono::steady_clock::now()).post() == ListenerResult::Stop) {
                return;
            }
            auto dispatcher = CCKeyboardDispatcher::get();
            dispatcher->dispatchKeyboardMSG(code, true, true, static_cast<double>(std::chrono::steady_clock::now().time_since_epoch().count()) / 1000000000);
        }];
    }];
}
